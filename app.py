...
if selected == "Home":
    ...
    st.write(
        "Web application that will visually present the IPR "
        "(Well Productivity Index Curve) and demand curves, "
        "allowing the user to evaluate the well's capacity "
        "based on the data he has provided."
    )
    animation_path = "resources/analisis_nodal.gif"
    st.image(animation_path,
             caption='IPR and Demand Curves',
             use_column_width=True)
    st.write("It is commonly accepted that wells are drilled "
             "and equipped for the primary purpose of extracting "
             "oil or gas from reservoirs. The movement of these "
             "fluids from the accumulations to and through the "
             "wellbore requires energy to compensate for frictional "
             "losses and bring them to the surface. "
             "The fluids initially travel through the reservoir, "
             "enter the wellbore, flow to the surface, pass "
             "through the pipeline system and are finally "
             "processed in the fluid separators located at the flow "
             "stations (Marcelo Hirschfeldt, 2009)")

if selected == "Data":
    st.write("In this section you must first upload your file "
             "containing the oil production with the respective dates.")
    ...
    df1
      ...
      == "Plots"):
    st.write("In this section you get the respective "
             "graph with the data entered in the Data section.")
    file = st.file_uploader("Upload your csv file with oil and water rate")
    df = pd.read_excel(file)
    df1 = pd.DataFrame(df)
    plots(df1)
...
if selected == "Calculations":
    st.write("This section is used to obtain the reservoir potential. "
             "Also, in the IPR Curve option you must load a "
             "file containing pwf data. "
             "To obtain the IPR curve by different methods.")
    if st.checkbox("Potential reservoir"):
        ...
        file2 = st.file_uploader(
            "Upload your csv file to Calculations/IPR CURVE")
    ...
    Data = namedtuple(
        "Input", "q_test pwf_test q pr pb sg_h2o API Q ID c wc")
    st.subheader("**Enter input values**")
    q_test = st.number_input("Enter q_test Value: ")
    ...
    st.write("This section is used to obtain the IPR and VLP curves, "
             "it is necessary to enter production data for a "
    ...
    Data = namedtuple(
        "Input", "Pwh WC SG_H2O API QT ID TVD MD C PR PB PWFT NVL")
    st.subheader("**Enter input values Well 1**")
    Pwh = st.number_input("Enter Pwh value: ")
    ...
    # Here the AOF is divided per 10 in order
    # to evaluate the pwf for these 10 different flow rates
    df2[columns[0]] = df1_a_n["oil_rate"].to_numpy()
    df2[columns[1]] = df2['q(bpd)'].apply(
        lambda x: pwf_darcy(QT, PWFT, x, PR, PB))
    df2[columns[2]] = Pwh
    ...
    df2[columns[7]] = (
            df2['THP(psia)'] + df2['Pgravity(psia)'] + df2['Pf(psia)'])
